...
function preset(context, _opts = {}) {
  ...
  const optionsMap = PLUGINS.map(plugin => option(plugin[0], plugin[1], plugin[2])).reduce(
    (acc, cur) => {
  ...
  const optionsTree = group(
    ...
    [
      ...
      group("properties", [optionsMap.consecutiveAdds, optionsMap.memberExpressions, optionsMap.propertyLiterals]),
      ...
      proxy("keepClassName", [optionsMap.mangle, optionsMap.deadcode])
    ],
    "some"
  );
  ...
  if (usedPlugins.size !== PLUGINS.length) {
    const unusedPlugins = PLUGINS.filter(plugin => !usedPlugins.has(plugin[0])).map(plugin => plugin[0]);
